    ...
    useEffect(() => {
        const fetchLichSuThanhToan = async () => {
            try {
                ...
                const response = await axios.get("http://localhost:5000/api/lich-su-thanh-toan", {
        ...
        <Box sx={{ padding: "20px" }}>
            ...
            <TableContainer component={Paper}>
                <Table>
                    ...
                    <TableBody>
                        {lichSuThanhToan.map((thanhToan) => (
                            <TableRow key={thanhToan.idthanhtoan}>
                                <TableCell>{thanhToan.idlohang}</TableCell>
                                <TableCell>{thanhToan.sotienthanhtoan.toLocaleString("vi-VN")}</TableCell>
                                <TableCell>{thanhToan.mota || "Không có mô tả"}</TableCell>
                                <TableCell>{new Date(thanhToan.ngaythanhtoan).toLocaleDateString("vi-VN")}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
